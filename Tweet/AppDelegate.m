...
- (void) saveTweets:(NSArray *)tweets
{
    if ([[[Twitter sharedInstance]session]authToken] && tweets) {
        ...
        if (fetchError) {
            ...
            NSManagedObject *tweet = nil;
            if (fetchResults.count > 0) {
                tweet = fetchResults.firstObject;
            }else{
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "PFYZ.Tweet" in the application's documents directory.
